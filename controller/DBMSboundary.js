...
DBMSboundary.prototype = {
    ...
    cercaPrenotazioniAccettate : function(email, cb) {
        ...
        "AND prenotazione.stato_prenotazione=1 AND (TIMESTAMPDIFF(day, NOW(), checkin)<= 1)";
        pool.query(qr, function (err, result) {
            if(err) throw err;
            if(result)
                cb(err, result);
    ...
    cercaDatiPrenotazione : function(email, id_pren, cb) {
        var qr = "SELECT DISTINCT ospite.* FROM prenotazione, ospite, alloggio "+ 
    ...
    /*cercaPrenotazione(id_pren, cb) {
        var qr = "SELECT * FROM prenotazione WHERE id_prenotazione='"+id_pren+"'";
        pool.query(qr, function (err, result) {
            if(err) throw err;
            if(result)
                cb(err, result);
    ...
    cercaPrenotazione(id_pren, cb) {
        var qr = "SELECT * FROM prenotazione, alloggio WHERE id_prenotazione='"+id_pren+"' "+
        "AND prenotazione.id_alloggio_fk=id_alloggio";
        pool.query(qr, function (err, result) {
            if(err) throw err;
            if(result)
                cb(err, result);
